...
本节所讲述的代码脚本在同级目录 [04-Intern-7B-Chat Lora 微调](./04-Qwen-7B-Chat%20Lora%20微调.py) 下，运行该脚本来执行微调过程，但注意，本文代码未使用分布式框架，微调 Qwen-7B-Chat 模型至少需要 24G 及以上的显存，且需要修改脚本文件中的模型路径和数据集路径。
