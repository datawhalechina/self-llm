...
## 数据集处理 
在本节我们使用开源的[Chat-甄嬛](https://github.com/KMnO4-zx/huanhuan-chat)项目作为示例，我们的目标是构建一个能够模拟甄嬛对话风格的个性化 LLM我们使用 **Dolly 格式** 构建数据集。Dolly 格式通常包含 `instruction` (指令)、`context` (上下文) 和 `response` (回答)。数据集在[这里](https://github.com/datawhalechina/self-llm/tree/master/dataset/huanhuan.json)，我们使用转换方法把这里的Alpaca格式转为了Dolly格式。
